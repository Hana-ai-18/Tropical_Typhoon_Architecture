...
def get_args():
    ...
    p.add_argument("--resume",       default=None, type=str,
                   help="Path to a checkpoint to resume training from -- "
                        "restores model, optimizer, scheduler, AMP scaler, "
                        "epoch, best_ade, and patience counter. Leave unset "
                        "to start fresh.")
...
def main(args):
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        try:
            ...
            print(f"  ⚠ Could not restore optimizer state ({e}) -- "
                  f"continuing with a freshly initialized optimizer.")
        try:
        ...
        try:
            if "scaler_state" in ck and args.use_amp:
                scaler.load_state_dict(ck["scaler_state"])
        except Exception as e:
            print(f"  ⚠ Could not restore AMP scaler state ({e})")
        start_epoch  = ck.get("epoch", 0) + 1
        best_ade     = ck.get("best_ade", float("inf"))
        patience_cnt = ck.get("patience_cnt", 0)
        print(f"  ↩ Resumed from epoch {start_epoch}  best_ade={best_ade:.1f}km  "
              f"patience={patience_cnt}/{args.patience}")
    elif args.resume:
        print(f"  ⚠ --resume path '{args.resume}' not found -- starting fresh instead.")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # Periodic checkpoint
        # [MỚI] Dùng file này cho --resume nếu training bị ngắt giữa 2 lần
        # validate (VD Kaggle session timeout) -- best-model checkpoint chỉ
        # lưu khi có cải thiện ADE, có thể cũ hơn nhiều so với epoch thực
        # tế đã train.
        if epoch % 100 == 0:
            torch.save({
                "epoch"      : epoch,
