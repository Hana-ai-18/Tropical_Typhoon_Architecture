...
try:
    from add_jtwc_overlay import draw_jtwc_overlay, load_jtwc_points
    HAS_JTWC_OVERLAY = True
except ImportError:
    HAS_JTWC_OVERLAY = False
import matplotlib.patches as mpatches
from matplotlib.lines import Line2D
...
def visualize_forecast(args):
    ...
    if HAS_JTWC_OVERLAY and args.jtwc_data_file:
        jtwc_key = f"{t_name}_{t_date}"
        jtwc_points = load_jtwc_points(jtwc_key, args.jtwc_data_file)
        if jtwc_points:
            draw_jtwc_overlay(
                ax_map, jtwc_points,
                transform=(ccrs.PlateCarree() if HAS_CARTOPY else None),
            ...
            print(f"  JTWC overlay: {len(jtwc_points)} points ({jtwc_key})")
...
if __name__ == "__main__":
    ...
    p.add_argument("--skip_existing",        action="store_true", default=False,
                   help="[batch_all] Bỏ qua nếu file output đã tồn tại "
                        "(hữu ích khi chạy lại sau khi bị gián đoạn).")
    p.add_argument("--jtwc_data_file",       default=None,
                   help="[optional] Đường dẫn tới file JSON chứa toạ độ "
                        "dự báo JTWC (xem add_jtwc_overlay.py, JTWC_DATA). "
                        "Nếu khớp tên storm + ngày dự báo với 1 key trong "
                        "file, tự động vẽ thêm đường JTWC (nét đứt cam) "
                        "lên ảnh output. Không truyền -> không vẽ JTWC "
                        "(hành vi giữ nguyên như trước).")
