...
STYLE = dict(
    ...
    gt_color      = "#1F5FBF",   # xanh dương — Actual Track (khớp ảnh mẫu)
    pred_color    = "#D62728",   # đỏ — Predicted (khớp ảnh mẫu)
    nwp_color     = "#FF8C00",   # cam — Ensemble NWP reference (mới)
    ens_color     = "#D62728",
    ens_alpha     = 0.05,
    marker_size   = 6,
    lw_main       = 2.0,
...
def _plot_on_ax(
    ...
    all_trajs_deg=None, errors_km=None,
    title="", dt_str="", pred_label="FM (mean)",
    ref_spread_km=None, nwp_deg=None, nwp_label="Ensemble NWP",
):
    transform = ccrs.PlateCarree() if HAS_CARTOPY else None
    # Viền TRẮNG quanh chữ/marker (đảo ngược so với bản dark, vốn viền
    # ĐEN quanh chữ sáng để nổi trên nền tối) — cho chữ đọc được trên nền
    ...
    # 4b. [MỚI] Ensemble NWP reference track (đọc từ file .txt bổ sung,
    # theo yêu cầu "kết hợp ngoài đường chính xác thì in thêm đường với
    # nhãn ensemble NWP"). nwp_deg là mảng [T', 2] tọa độ (lon, lat) tại
    # các mốc giờ có trong file NWP cho đúng storm/thời điểm này; có thể
    # ngắn hơn pred_deg (NWP thường chỉ có tới 1 vài mốc, ví dụ chỉ tới
    # 72h nhưng thưa hơn best-track 6h/bước) nên KHÔNG ép cùng độ dài với
    # gt_deg/pred_deg — vẽ đúng số điểm có sẵn. None nếu không có dữ
    # liệu NWP cho storm này (phần lớn trường hợp), khi đó không vẽ gì.
    if nwp_deg is not None and len(nwp_deg) > 0:
        nwp_lon = np.concatenate([[cur_pos[0]], nwp_deg[:, 0]])
        nwp_lat = np.concatenate([[cur_pos[1]], nwp_deg[:, 1]])
        _plot(nwp_lon, nwp_lat, fmt="^--",
              color=STYLE.get("nwp_color", "#FF8C00"), linewidth=STYLE["lw_thin"],
              markersize=STYLE["marker_size"] - 1,
              markeredgecolor="white", markeredgewidth=1.0,
              zorder=8.5, path_effects=outline)
    ...
    if nwp_deg is not None and len(nwp_deg) > 0:
        track_handles.append(
            Line2D([0], [0], color=STYLE.get("nwp_color", "#FF8C00"), lw=1.8,
                   linestyle="--", marker="^", markersize=5, label=nwp_label)
        )
    track_handles += [
        mpatches.Patch(facecolor=STYLE["cone_50_fill"], alpha=0.5,
                       label="50% prob. cone (center track)"),
...
def parse_storm_selection_file(path: str):
    """
    [MỚI] Đọc file chỉ định các storm cần visualize. Hỗ trợ 2 dạng dòng
    trộn lẫn trong cùng 1 file (đúng ảnh mẫu người dùng cung cấp, có cả
    2 kiểu):

      (a) TÊN + GIỜ + NGÀY -- lọc CHÍNH XÁC đúng 1 thời điểm dự báo:
            EWINIAR 06:00 25/05/2024
            LINFA 12:00  18/06/2009      (nhiều khoảng trắng cũng OK)

      (b) Chỉ TÊN (1 token duy nhất) -- lấy TOÀN BỘ thời điểm dự báo có
          sẵn của storm đó trong dataset, giống hệt --storm_filter:
            YANCU
            LINFA
            MOLAVE

    (Lưu ý: nếu 1 tên bão xuất hiện cả ở dạng (a) lẫn dạng (b) trong
    cùng file, dạng (b) sẽ thắng cho storm đó — vì "lấy toàn bộ" đã bao
    trùm mọi thời điểm cụ thể của (a).)

    Trả về tuple (wanted_pairs, wanted_names_only):
      - wanted_pairs: set (NAME_UPPER, "YYYY-MM-DD HH:MM") cho dạng (a)
      - wanted_names_only: set NAME_UPPER cho dạng (b)
    """
    wanted_pairs = set()
    wanted_names_only = set()
    with open(path, "r", encoding="utf-8") as f:
        for line_no, raw in enumerate(f, 1):
            ...
            parts = line.split()
            if len(parts) == 1:
                wanted_names_only.add(parts[0].strip().upper())
                continue
            if len(parts) < 3:
                print(f"  ⚠ Bỏ qua dòng {line_no} (cần 1 token [chỉ tên] hoặc "
                      f"≥3 token [tên giờ ngày]): {raw!r}")
                continue
            name, time_str, date_str = parts[0], parts[1], parts[2]
            try:
                dt = datetime.strptime(f"{date_str} {time_str}", "%d/%m/%Y %H:%M")
            except ValueError:
                print(f"  ⚠ Bỏ qua dòng {line_no} (sai định dạng ngày/giờ): {raw!r}")
                continue
            fdate = dt.strftime("%Y-%m-%d %H:%M")
            wanted_pairs.add((name.strip().upper(), fdate))
    return wanted_pairs, wanted_names_only


def parse_nwp_data_file(path: str):
    """
    [MỚI] Đọc file .txt chứa đường đi NWP tham chiếu do người dùng tự
    tổng hợp, theo đúng định dạng ảnh mẫu HARD_STROM.txt:

        =========================================
        BAO: EWINIAR
        Thoi gian du bao: 2024-05-25 06:00
        =========================================
    ...
    Mỗi khối "BAO: <TEN>" + "Thoi gian du bao: <YYYY-MM-DD HH:MM>" xác
    định 1 storm/forecast-date cụ thể (khớp đúng key (name, fdate) dùng
    ở parse_storm_selection_file / iterate_all_storms). Bảng Hour/Lat/
    Lon theo sau liệt kê track NWP tại các mốc giờ kể từ NOW (hour=0).

    Trả về dict {(NAME_UPPER, "YYYY-MM-DD HH:MM"): np.ndarray[T,2]}
    với mỗi hàng là (lon, lat) tại các hour > 0 (bỏ hour=0 vì đó chính
    là NOW, đã có sẵn từ obs_deg[-1], không cần lặp lại trong nwp_deg).
    """
    result = {}
    cur_name, cur_fdate = None, None
    cur_rows = []
    header_seen = False

    def _flush():
        if cur_name is not None and cur_fdate is not None and cur_rows:
            arr = np.array([[lon, lat] for _, lat, lon in sorted(cur_rows)], dtype=np.float64)
            result[(cur_name, cur_fdate)] = arr
    ...
    with open(path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            if line.startswith("BAO:"):
                _flush()
                cur_name = line.split("BAO:", 1)[1].strip().upper()
                cur_fdate = None
                cur_rows = []
                header_seen = False
                continue
            if line.startswith("Thoi gian du bao:"):
                raw_dt = line.split(":", 1)[1].strip()
                # Cho phép cả "2024-05-25 06:00" (đã đúng định dạng) lẫn
                # "2024-05-25" (thiếu giờ, mặc định 00:00) để linh hoạt.
                try:
                    ...
                    try:
                        ...
                        print(f"  ⚠ Không đọc được ngày NWP: {raw_dt!r}")
                        continue
                cur_fdate = dt.strftime("%Y-%m-%d %H:%M")
                continue
            if line.startswith("=") or line.lower().startswith("hour"):
                header_seen = True
                continue
            if header_seen and cur_name is not None:
                toks = line.split()
                if len(toks) >= 3:
                    try:
                        hour = float(toks[0])
                        lat = float(toks[1])
                        lon = float(toks[2])
                        if hour > 0:  # bỏ hour=0 (NOW), không cần trong nwp_deg
                            cur_rows.append((hour, lat, lon))
                    except ValueError:
                        continue
    _flush()
    return result
...
def visualize_batch_all_storms(args):
    ...
    # [MỚI] Lọc theo file --storm_datetime_file, hỗ trợ 2 dạng dòng
    # (xem docstring parse_storm_selection_file): dòng "TEN GIỜ NGÀY"
    # lọc đúng 1 thời điểm cụ thể; dòng chỉ "TEN" giữ lại TOÀN BỘ thời
    # điểm có sẵn của storm đó (giống --storm_filter). Cần thiết vì
    # cùng 1 tên bão có thể xuất hiện nhiều năm khác nhau trong dataset.
    if args.storm_datetime_file:
        wanted_pairs, wanted_names_only = parse_storm_selection_file(args.storm_datetime_file)
        print(f"  --storm_datetime_file: {len(wanted_pairs)} (storm, date) cụ thể + "
              f"{len(wanted_names_only)} storm lấy toàn bộ thời điểm")
        new_storms = {}
        found_pairs = set()
        for name, windows in storms.items():
            if name in wanted_names_only:
                # Dạng (b): lấy toàn bộ thời điểm của storm này.
                kept = list(windows)
            else:
                # Dạng (a): chỉ giữ đúng thời điểm được liệt kê.
                kept = [(idx, fdate) for idx, fdate in windows if (name, fdate) in wanted_pairs]
            for idx, fdate in kept:
                found_pairs.add((name, fdate))
            if kept:
                new_storms[name] = kept
        storms = new_storms
        missing = wanted_pairs - found_pairs
        if missing:
            print(f"  ⚠ {len(missing)} mục trong file không khớp dữ liệu (không tìm thấy trong dataset):")
            for name, fdate in sorted(missing):
                print(f"      {name} @ {fdate}")
        missing_names = wanted_names_only - set(storms.keys())
        if missing_names:
            print(f"  ⚠ {len(missing_names)} tên storm không tìm thấy trong dataset: "
                  f"{sorted(missing_names)}")

    # [MỚI] Đọc file NWP tham chiếu (nếu có) để vẽ thêm đường "Ensemble
    # NWP" lên mỗi hình, theo đúng yêu cầu "kết hợp ngoài đường chính
    # xác thì in thêm đường với nhãn ensemble NWP". Optional — nếu
    # không truyền --nwp_data_file, nwp_lookup rỗng, không vẽ gì thêm.
    nwp_lookup = {}
    if args.nwp_data_file:
        nwp_lookup = parse_nwp_data_file(args.nwp_data_file)
        print(f"  --nwp_data_file: {len(nwp_lookup)} track NWP tham chiếu đã nạp")
    ...
    for storm_name, windows in storms.items():
        for dataset_idx, fdate in windows:
            target = dset[dataset_idx]
            nwp_deg = nwp_lookup.get((storm_name, fdate))  # None nếu không có NWP cho mốc này
            ...
            for model_name, model_kind, checkpoints in model_groups:
                ...
                try:
                    ...
                    if ens_deg is not None:
                        all_pts.append(ens_deg.reshape(-1, 2))
                    if nwp_deg is not None and len(nwp_deg) > 0:
                        all_pts.append(nwp_deg)  # đảm bảo bản đồ đủ rộng để chứa cả đường NWP
                    all_deg = np.vstack(all_pts)
...
if __name__ == "__main__":
    ...
    p.add_argument("--storm_filter",         nargs="+", default=None,
                   help="[batch_all, optional] Chỉ chạy các storm này "
                        "(tên viết hoa, ví dụ RITA WIPHA) — không truyền "
                        "thì chạy TOÀN BỘ storm trong dataset.")
    p.add_argument("--storm_datetime_file",  default=None,
                   help="[batch_all, optional] File .txt chỉ định CHÍNH XÁC "
                        "từng (tên bão, thời điểm dự báo) cần vẽ, mỗi dòng "
                        "dạng 'TEN HH:MM DD/MM/YYYY', ví dụ "
                        "'EWINIAR 06:00 25/05/2024'. Khác --storm_filter "
                        "(chỉ lọc theo tên, giữ mọi thời điểm): file này lọc "
                        "đúng từng thời điểm cụ thể, cần thiết khi cùng 1 "
                        "tên bão xuất hiện nhiều năm trong dataset.")
    p.add_argument("--nwp_data_file",        default=None,
                   help="[batch_all, optional] File .txt chứa track NWP "
                        "tham chiếu tự tổng hợp, theo định dạng khối "
                        "'BAO: <TEN>' / 'Thoi gian du bao: <ngày giờ>' / "
                        "bảng Hour-Latitude-Longitude. Nếu khớp đúng "
                        "(tên, thời điểm) của 1 hình đang vẽ, đường NWP sẽ "
                        "được vẽ thêm lên bản đồ với nhãn 'Ensemble NWP'.")
    p.add_argument("--skip_existing",        action="store_true", default=False,
                   help="[batch_all] Bỏ qua nếu file output đã tồn tại "
                        "(hữu ích khi chạy lại sau khi bị gián đoạn).")
