...
from Model.st_trans_model import STTrans
from Model.paper_baseline_model import PaperBaseline
# [MỚI] MGTCF (multi-generator GAN) và Phys-Diff (latent diffusion +
# PIGA) — 2 baseline generative bổ sung, xem Model/mgtcf_model.py /
# Model/physdiff_model.py cho chi tiết kiến trúc. Cả 2 đều có
# sample(batch_list, num_ensemble=..., **kwargs) -> (pred, zeros,
# all_trajs) cùng convention với TCFlowMatching/STTrans/PaperBaseline
# (đã verify bằng test end-to-end trước khi bàn giao), nên toàn bộ
# phần đánh giá phía dưới không cần sửa gì khác ngoài _build_model().
...
# định cũ (không truyền --model_type vẫn ra đúng TCFlowMatching, không
# đổi hành vi cho mọi lệnh gọi evaluate_full.py đã có từ trước).
MODEL_TYPES = ["fm", "st_trans", "lstm", "gru", "rnn", "mgtcf", "physdiff"]
...
def _build_model(model_type: str, model_cfg: dict, device):
    ...
    if model_type == "fm":
        ...
        cfg.setdefault("model_type", model_type)
        return PaperBaseline(**cfg).to(device)
    elif model_type == "mgtcf":
        # [MỚI] MGTCFModel.__init__ KHÔNG nhận tham số "model_type" (khác
        # PaperBaseline) — model_cfg lưu bởi train_mgtcf.py vốn không
        # chứa key này (xem _infer_model_type_from_checkpoint: model_type
        # thật của MGTCF/PhysDiff nằm ở ck["model_type"] cấp ngoài, không
        # phải trong model_cfg), nhưng vẫn .pop() phòng hờ để an toàn nếu
        # sau này có ai vô tình thêm nhầm key này vào model_cfg khi lưu
        # checkpoint thủ công.
        cfg = dict(model_cfg)
        cfg.pop("model_type", None)
        return MGTCFModel(**cfg).to(device)
    elif model_type == "physdiff":
        cfg = dict(model_cfg)
        cfg.pop("model_type", None)
        return PhysDiffModel(**cfg).to(device)
    else:
        raise ValueError(f"Unknown --model_type: {model_type!r} "
                          f"(phải là 1 trong {MODEL_TYPES})")
...
def _infer_model_type_from_checkpoint(ck: dict, cli_model_type: str) -> str:
    """
    [MỚI] Nếu checkpoint tự ghi lại kiến trúc thật, ưu tiên nó thay vì
    --model_type CLI người dùng gõ tay — tránh trường hợp gõ nhầm
    --model_type lstm nhưng checkpoint thật là gru (2 kiến trúc có cùng
    shape tensor phần lớn layer, load_state_dict(strict=False) có thể
    "load được" một phần rồi âm thầm cho kết quả sai thay vì báo lỗi
    rõ ràng). Cảnh báo rõ nếu 2 nguồn xung đột; vẫn ưu tiên checkpoint
    vì đó là nguồn đáng tin hơn (ghi tại thời điểm train, không phụ
    thuộc người dùng gõ đúng CLI mỗi lần eval).

    HAI QUY ƯỚC LƯU KHÁC NHAU tồn tại trong codebase (do 2 nhóm baseline
    được viết ở 2 thời điểm khác nhau) — hàm này đọc CẢ HAI, ưu tiên
    theo thứ tự dưới đây:
      1. model_cfg["model_type"] (PaperBaseline: "lstm"/"gru"/"rnn",
         đã lowercase sẵn, khớp trực tiếp MODEL_TYPES).
      2. ck["model_type"] cấp NGOÀI model_cfg (MGTCFModel/PhysDiffModel,
         train_mgtcf.py/train_physdiff.py lưu "MGTCF"/"PhysDiff" viết
         hoa/PascalCase — cần .lower() để khớp MODEL_TYPES).
    """
    model_cfg = ck.get("model_cfg") or {}
    ckpt_type = model_cfg.get("model_type")

    if not ckpt_type:
        # Fallback: đọc ck["model_type"] cấp ngoài (quy ước của
        # MGTCFModel/PhysDiffModel), chuẩn hoá về lowercase để khớp
        # đúng MODEL_TYPES ("MGTCF"->"mgtcf", "PhysDiff"->"physdiff").
        outer_type = ck.get("model_type")
        if outer_type:
            ckpt_type = outer_type.lower()
