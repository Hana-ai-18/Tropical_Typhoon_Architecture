...
def ate_cte_paper_formula(pred_norm: torch.Tensor, gt_norm: torch.Tensor):
    ...
    projected onto/across a GT-implied heading direction via a 2D
    dot/cross product) — NOT the same math as FM's ate_cte_full above
    (full spherical forward-azimuth/haversine via atan2).

    [FIX-INDEXING, second bug found] _ate_cte_tensors' heading is a
    FORWARD difference: dir_raw[k] = gt[k+1] - gt[k] for k=0..T-2, with
    the LAST step's direction simply repeated for k=T-1 (see its own
    `torch.cat([gt_deg[1:]-gt_deg[:-1], gt_deg[-1:]-gt_deg[-2:-1]])`).
    This means ATE/CTE ARE validly defined at step 0 (using the
    direction TOWARD step 1) — unlike FM's ate_cte_full, which needs a
    PRIOR point and is only defined from step 1 onward (backward
    difference: bearing FROM step k-1 TO step k). These are genuinely
    different index conventions, not just different formulas. An
    earlier version of this function sliced off index 0 to match
    ate_cte_full's [T-1,B] contract, which was WRONG for this formula —
    it silently shifted every non-FM model's ATE/CTE by one lead_time
    (12h's reported value was actually 6h's ground truth, etc.) and
    dropped 72h (step0=T-1) entirely, since it fell outside the
    now-shifted range. Returns the FULL [T, B] undropped — the
    call site handles the differing validity range per formula
    (is_fm ? step0>=1 : step0>=0) instead of this function faking a
    common [T-1,B] shape that doesn't actually apply here.
    """
    T = min(pred_norm.shape[0], gt_norm.shape[0])
    if T < 1:
        z = pred_norm.new_zeros(1, pred_norm.shape[1])
        return z, z
    return _ate_cte_tensors(pred_norm[:T], gt_norm[:T])  # [T, B], valid from step0=0
...
def evaluate_one_model(model, loader, device, model_name: str,
    ...
    for bi, batch in enumerate(loader):
        ...
        if is_fm:
            ate, cte = ate_cte_full(pd, gd)           # [T-1, B] -- ate[k] = error at step k+1 (0-indexed), NOT valid at step0=0
        else:
            ate, cte = ate_cte_paper_formula(pred[:T], gt[:T, :, :2])  # [T, B] -- ate[k] = error at step k (0-indexed), valid from step0=0
        T_valid = ate.shape[0]                        # FM: T-1 | others: T
        ...
        for b in range(B):
            ...
            for step0 in range(T):           # step0 = 0-indexed step, 0..T-1
                ...
                # ADE (d) có đủ giá trị cho MỌI lead_time 1..T.
                #
                # [FIX-ATE-CTE-VALIDITY-RANGE, second index bug] ATE/CTE's
                # valid range and array index DEPEND ON WHICH FORMULA is in
                # use, not a single fixed rule:
                #   - FM (ate_cte_full, backward-difference bearing): needs a
                #     PRIOR point, undefined at step0=0 (=6h). Valid
                #     step0=1..T-1, array index = step0-1.
                #   - Every other model (ate_cte_paper_formula /
                #     _ate_cte_tensors, forward-difference heading): valid
                #     from step0=0 straight through step0=T-1 (=72h), array
                #     index = step0 directly, NO shift. An earlier version of
                #     this code applied FM's has_atecte/ate_i rule (step0>=1,
                #     ate_i=step0-1) to ALL models uniformly -- for non-FM
                #     models this wrongly discarded the true 6h ATE/CTE value,
                #     silently mislabeled every OTHER horizon's value as the
                #     PREVIOUS horizon (e.g. 12h's record actually held the
                #     value computed at 6h), and dropped 72h off the end
                #     entirely (ate_i=T-1 index out of bounds after the
                #     shift). This is a SEPARATE bug from the formula choice
                #     itself -- fixing the formula alone (routing non-FM
                #     models to ate_cte_paper_formula) was not sufficient
                #     without also fixing this indexing.
                if is_fm:
                    has_atecte = step0 >= 1      # ate/cte undefined at step0=0 (backward-diff needs a prior point)
                    ate_i = step0 - 1            # array index into ate_cte_full's [T-1,B] result
                else:
                    has_atecte = step0 < T_valid # ate/cte defined step0=0..T-1 (forward-diff, T_valid==T here)
                    ate_i = step0                # array index into ate_cte_paper_formula's [T,B] result, NO shift
                records.append({
                    "model":     model_name,
                    "seed":      seed,
                    "storm":     storm_key,
